...
def train_one_epoch(model, dataloader, optimizer, criterion, device):
    ...
    for inputs, labels in dataloader:
        ...
        if isinstance(outputs, tuple):
            ...
            loss_main = criterion(main_output, labels)  # Compute main loss
            loss_aux = criterion(aux_output, labels) # Compute auxiliary loss
            loss = loss_main + 0.4 * loss_aux       # Combine losses (with weighting for aux)
            preds = torch.argmax(main_output, dim=1) # # Get predicted class (highest score)
...
def run_crossval_and_save_models(dataset, model_dir, device, k_folds=5, batch_size=64, max_epochs=15, patience=5):
    ...
    final_model = InceptModel().to(device)
    ...
    for fold, (train_idx, val_idx) in enumerate(kfold.split(np.zeros(len(labels)), labels)):
        ...
        model = InceptModel() # Initialize a new model instance for this fold
...
model_dir="5models_Incept-5types_170k" # Define directory where models trained for each fold will be saved
...
def ensemble_test(test_loader, model_dir, device):
    ...
    for i in range(5):
        ...
        model = InceptModel(num_classes=5)  # Instantiate the same model architecture used during training
...
def plot_roc_curve(y_true, y_probs, model_name):
    ...
    plt.title(f'ROC Curve - {model_name} (Multiclass)', fontsize=20)
...
plot_roc_curve(y_true, y_pred, "Test InceptionV3")
plot_score_distributions(y_true, y_pred, "Test InceptionV3")

# Save the predictions and labels into a .npz file for later analysis or plotting
model_name = "ROC-Inception-170k"
